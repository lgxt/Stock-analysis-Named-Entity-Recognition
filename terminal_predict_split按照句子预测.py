...
def predict_online():
    ...
    with graph.as_default():
        ...
        # while True:
        start = datetime.now()
        fl = codecs.open('predict_split.csv', 'a+', 'gbk')
        ...
        with open('all.csv', 'r', encoding="utf-8") as f:
            ...
            for line in lines:
                ...
                for sentence in sentences:
                    ...
                    #     # print("++++")
                    toks = list(zip(sentence, pred_label_result[0]))
                    seller, buyer, target = identify_entities(toks)
                    for s in seller:
                        if s not in sellers:
                            sellers.append(s)
                    for b in buyer:
                ...
                writer.writerow([str(line[0]), line[1], ','.join(list(sellers)), ','.join(list(buyers)), ','.join(list(targets))])
