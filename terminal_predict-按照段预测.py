...
# 修改运行环境
model_dir = '/data/lucas/Bert+BiLSTM+CRF-linux-GuQuanAnalyse3-newBert-Predict/output'
bert_dir = '/data/lucas/Bert+BiLSTM+CRF-linux-GuQuanAnalyse3-newBert-Predict/lucas_bert'
...
def predict_online():
    ...
    with graph.as_default():
        ...
        # sentence = str(input())
        # linliquan 写入csv文件
        fl = codecs.open('predict.csv', 'a+', 'gbk')
        writer = csv.writer(fl)
        writer.writerow(['id', 'content', 'seller', 'buyer', 'target'])
        # linliquan 读入csv文件 2020-08-14
        with open('all.csv', 'r', encoding="utf-8") as f:
            ...
            for line in lines:
                ...
                if len(line[1]) < 2:
                    print(line[1])
                    break
                sentence = tokenizer.tokenize(line[1])
                ...
                try:
                    toks = list(zip(sentence, pred_label_result[0]))
                    # print(toks)
                    sellers, buyers, targets = identify_entities(toks)
                    # print(sellers)
                    # print(buyers)
                    # print(targets)
                    writer.writerow([str(line[0]), line[1], ','.join(sellers), ','.join(buyers), ','.join(targets)])
                    # data.append((str(line[0]), sentence, ','.join(sellers), ','.join(buyers), ','.join(targets)))
                except:
                    print(sentence)
...
def identify_entities(toks):
    """
    通过labels将seller，buyer，target读取出来
    :param toks:
    :return:
    """
    def ner_labels(toks, entity, i, labels_1,flag):
        entity += toks[i][0]
        for j in range(i + 1, len(toks)):
            if len(toks[j][1].split('-')) == 2 and toks[j][1].split('-')[1] == labels_1 and flag:
                entity += toks[j][0]
            else:
                flag = False
        return entity
    ...
    for i in range(len(toks)):
        labels = toks[i][1].split('-')
        if labels[0] == 'B':
            flag = True
            if labels[1] == 'seller':
                seller = ner_labels(toks, entity, i, labels[1], flag)
                sellers.add(seller)
            elif labels[1] == 'buyer':
                buyer = ner_labels(toks, entity, i, labels[1], flag)
                buyers.add(buyer)
            elif labels[1] == 'target':
                target = ner_labels(toks, entity, i, labels[1], flag)
                targets.add(target)
            else:
                break
    # print(sellers)
    # print(buyers)
    # print(targets)
    return list(sellers), list(buyers), list(targets)
